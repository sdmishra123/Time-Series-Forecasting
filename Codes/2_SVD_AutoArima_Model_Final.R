...
forecast.reduced.arima <- function(reduced.train){
  ...
  c1.forecast <- auto.arima(c1,stepwise = F,approximation = F,seasonal=TRUE,lambda=lambda)
  c1.new <- c1.forecast$fitted
  c1.pred <- forecast(c1.forecast,h = 4)
  
  c2 <- ts(reduced.train[,2],frequency = 52)
  lambda <- BoxCox.lambda(c1)
  c2.forecast <- auto.arima(c2,stepwise = F,approximation = F,seasonal=TRUE,lambda=lambda)
  c2.new <- c2.forecast$fitted
  c2.pred <- forecast(c2.forecast,h = 4)
  
  c3 <- ts(reduced.train[,3],frequency = 52)
  lambda <- BoxCox.lambda(c1)
  c3.forecast <- auto.arima(c3,stepwise = F,approximation = F,seasonal=TRUE,lambda=lambda)
...
Actual_Predicted_trans_v1$Type <- "AutoArima" 

write.csv(Actual_Predicted_trans_v1,"Actual_Predicted_trans_AutoArima.csv")
