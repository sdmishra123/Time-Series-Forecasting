...
forecast.reduced.tslm <- function(reduced.train){
  
  #For TSLM we need to store the time series and dates in a data frame
  reduced_df<-data.frame(train.processed[,1],reduced.train[,1],reduced.train[,2],reduced.train[,3])
  names(reduced_df)<-c("Date","c1","c2","c3")
  
  #lambda <- BoxCox.lambda(ts(reduced_df$c1,frequency = 52))
  c1.forecast <- tslm( ts(reduced_df$c1,frequency = 52) ~ trend,reduced_df) # Adding just trend improves the model performance
  c1.new <- c1.forecast$fitted
  c1.pred <- forecast(c1.forecast,h = 4)
  
  #lambda <- BoxCox.lambda(ts(reduced_df$c2,frequency = 52))
  c2.forecast <- tslm( ts(reduced_df$c2,frequency = 52) ~ trend+season,reduced_df) # Adding both trend and season improves rsquare but reduces adjusted rsquare as df increases 
  c2.new <- c2.forecast$fitted
  c2.pred <- forecast(c2.forecast,h = 4)
  
  #lambda <- BoxCox.lambda(ts(reduced_df$c3,frequency = 52))
  c3.forecast <- tslm( ts(reduced_df$c3,frequency = 52) ~ trend+season,reduced_df) # Adding both trend and season improves rsquare and reduces adjusted rsquare slightly as df increases
...
dept.forecast.tslm <- function(train.processed){
  ...
  arima.list <- forecast.reduced.tslm(reduced.train)
...
dept.1.list     <- dept.forecast.tslm(train.processed)
...
Actual_Predicted_trans_v1$Type <- "TSLM" 
head(Actual_Predicted_trans_v1)

write.csv(Actual_Predicted_trans_v1,"Actual_Predicted_trans_TSLM.csv")
